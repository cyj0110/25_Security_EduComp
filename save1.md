...
**[RAID와 카빙]**
포렌식에서 **카빙(Carving)** 이란,
파일 시스템의 메타데이터(파일명, 위치 정보 등)가 없거나 손상된 상태에서도 저장매체의 원시 데이터(raw data)를 분석해 파일을 복구하는 기법

상용 소프트웨어 서버에는 RAID로 구성된 워크스테이션이 많음. (FTK, AXIOM은 RAID X)

X-ways에서는 RAID로 파일 시스템을 불러오고, 파일 시그니처를 통해 카빙을 수행할 수 있다.
```
[RAID]
RAID 0: 목적은 저장소의 크기를 증가시키는 것, 2개 이상의 저장소를 가짐(1TB + 1TB = 약1.8TB), 읽기/쓰기 속도 빠름
RAID 1: 목적은 백업의 중요성이 클때 사용하기 위함, 1TB + 1TB에서 한 디스크는 다른 디스크를 미러링, 즉 가용성에 문제 없음
RAID 0과 1 둘다 사용 시 용량과 백업 둘다 확보
RAID 4: 최소 디스크 개수가 3개 이상부터 사용 가능, 패리티를 사용할 디스크 하나를 지정
RAID 5: 최소 디스크 개수가 3개 이상부터 사용 가능, 패리티는 각 디스크에 분산되어 있어 어떤 디스크가 손상되어도 가용성 확보 가능
```
