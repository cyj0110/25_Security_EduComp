...
WebDAV(Web Distributed Authoring and Versioning)는 HTTP 프로토콜의 확장으로, RFC 4918에 정의된 **파일 관리 및 협업을 위한 프로토콜**이다. 사용자가 원격 웹 서버에서 **파일을 생성, 수정, 이동, 삭제**할 수 있도록 지원하며, 주요 기능은 다음과 같다:
...
WebDAV는 원격 파일 공유와 협업 애플리케이션(예: 문서 편집, 클라우드 스토리지)에 널리 사용되지만, 잘못된 구성이나 취약한 권한 설정으로 인해 보안 위협에 노출될 수 있다.

**권한에 따른 WebDAV 보안 위협**

WebDAV의 보안 위협은 주로 권한 설정의 부적절함과 관련이 있다. 아래는 권한별 주요 위협이다:

1. 읽기 권한(Read Permission)
    - 위협: 인증 없이 읽기 권한이 부여된 경우, 공격자가 민감한 파일(예: 구성 파일, 데이터베이스 백업)을 다운로드할 수 있음.
    - 예시: PROPFIND 메서드를 통해 디렉토리 구조를 나열하거나, GET 메서드로 민감한 파일을 획득.
    - 영향: 데이터 유출, 내부 시스템 정보 노출.
2. 쓰기 권한(Write Permission)
    - 위협: 쓰기 권한이 부여된 경우, 공격자가 악성 파일(예: 웹셸, 리버스 쉘)을 업로드하여 서버를 장악하거나 임의 코드 실행(RCE)을 시도.
    - 예시: PUT 메서드를 사용해 PHP/ASP 파일을 업로드하고, 이를 실행하여 서버 제어.
    - 영향: 서버 완전 장악, 백도어 설치, 데이터 삭제/변조.
3. 실행 권한(Execute Permission)
    - 위협: 업로드된 파일에 실행 권한이 부여되거나, 서버가 특정 확장자(예: .php, .asp)를 실행하도록 설정된 경우, 공격자가 업로드한 악성 스크립트를 실행.
    - 예시: WebDAV를 통해 업로드된 PHP 웹셸이 웹 서버에서 실행되어 공격자가 명령을 실행.
    - 영향: 시스템 명령 실행, 권한 상승, 네트워크 내부 침투.
4. 인증 우회 또는 약한 인증
    - 위협: 기본 인증(Basic Auth)이 약한 암호로 설정되거나, 인증이 전혀 없는 경우, 공격자가 무차별 대입 공격(Brute Force)으로 접근 가능.
    - 예시: Hydra와 같은 도구로 약한 암호를 크랙하거나, 인증 없이 WebDAV 디렉토리에 접근.
    - 영향: 무단 접근, 데이터 손실, 악성 코드 실행.
5. 구성 취약점
    - 위협: WebDAV가 불필요한 디렉토리에 활성화되거나, 특정 HTTP 메서드(예: PUT, MOVE)가 제한 없이 허용된 경우.
    - 예시: WebDAV가 루트 디렉토리에 활성화되어 공격자가 시스템 파일을 수정.
    - 영향: 서버 전체에 대한 무단 접근 및 조작.
6. 최근 제로데이 취약점 (CVE-2025-33053)
    - 위협: WebDAV의 원격 코드 실행(RCE) 취약점으로, 특수 제작된 URL을 통해 악성 코드를 실행 가능.
    - 예시: 피싱 이메일에 포함된 URL 파일을 피해자가 클릭하면 WebDAV 서버에서 악성 페이로드가 실행.
    - 영향: 서버 장악, 데이터 유출, 추가 악성 코드 배포.
...
## 버프스위트 활용하여 webdav 취약점 진단

Burp Suite는 웹 애플리케이션의 보안 취약점을 점검하고 침투 테스트(penetration testing)를 수행할 때 사용하는 전문 보안 도구이다.

-proxy 설정 필요. (8080번 포트)

system() <- shell 명령어 - 웹 쉘의 위험성이 있음.
...
원격으로 악성 코드를 실행할 수 있는 보안 취약점을 의미한다. 이 취약점은 공격자가 시스템의 권한을 탈취하여 악의적인 행위를 수행할 수 있도록 허용하기 때문에 매우 위험한 취약점으로 간주된다. 특히 윈도우 애플리케이션에서 RCE 취약점이 발생하면, 조직의 주요 데이터와 시스템의 안정성이 심각하게 위협받을 수 있다.

### **RCE 취약점의 주요 원인**

1. **입력 값 검증의 부재 또는 미흡**
    - 사용자가 입력한 데이터를 제대로 검증하지 않는 경우, 공격자가 악성 코드를 포함한 데이터를 전달하여 시스템에서 실행되도록 유도할 수 있다.
    - 예: SQL 주입(SQL Injection)이나 OS 명령어 주입(Command Injection).
2. **취약한 라이브러리 사용**
    - 애플리케이션에서 사용하는 서드파티 라이브러리나 컴포넌트에 RCE 취약점이 존재할 경우, 이를 악용해 원격 코드 실행이 이루어질 수 있다.
    - 예: Apache Log4j에서 발견된 **Log4Shell** 취약점(CVE-2021-44228).
3. **메모리 관리 취약점**
    - 메모리 버그, 예를 들어 **버퍼 오버플로(Buffer Overflow)** 또는 **사용 후 해제(UAF: Use-After-Free)** 문제가 있는 경우, 이를 통해 악성 코드 실행이 가능하다.
4. **잘못된 구성**
    - 시스템 또는 애플리케이션 설정에서 잘못된 권한 부여나 보안 설정이 이루어진 경우, 공격자가 이를 악용하여 코드 실행 권한을 얻을 수 있다.
  
### **RCE 취약점의 주요 위험**

- **권한 탈취 및 정보 유출**: 공격자가 관리자 권한을 얻어 민감한 데이터에 접근하거나 시스템을 제어할 수 있다.
- **랜섬웨어 배포**: 공격자는 악성 코드를 실행하여 데이터를 암호화한 후 금전을 요구할 수 있다.
- **시스템 파괴**: 시스템 설정을 변경하거나 삭제하여 복구 불가능한 상태로 만들 수도 있다.
  
---

## 윈도우 애플리케이션 RCE 취약점 사례 - HFS 입력값 검증 미흡

### **HFS 2.3b RCE 취약점의 개요**

- **취약점 코드**: CVE-2014-6287
- **취약점 유형**: 명령 주입(Command Injection)으로 인한 원격 코드 실행
- **발생 원인**:
    - HTTP 요청을 처리하는 과정에서 입력 값에 대한 적절한 검증이 이루어지지 않았다.
    - 특히, 특정 HTTP 요청에서 `URL 인코딩`이 제대로 처리되지 않아 공격자가 악의적인 명령을 포함한 입력을 서버에서 실행할 수 있었다.
 
---

## 윈도우 애플리케이션 RCE 취약점 사례 - 시스템 침투 및 공격 원리 이해
...
**WireShark에서 pcap 확인**
display에 http 검색한 후, 패킷을 TCP 따라가기 후 저장된 내용을 URL Decode시, 공격 구문 확인 가능.

```
GET/?search= {.exec|wscript.exe+//B+//NOLOGO+%TEMP%\YEWEFCGdpZQl.vbs.}HTTP/1.1
```
-Visual Basic Script = 액셀 매크로, PPT 매크로
윈도우 대상 악성코드 배포!

윈도우에서 실행할 수 있는 명령어 - wscript (vbs..), powershell, netsh, wmi...

### **취약점의 동작 원리**

HFS는 기본적으로 HTTP 요청을 받아 파일을 처리하거나 경로를 탐색하는 기능을 제공한다. 하지만 HFS 2.3b에서는 다음과 같은 문제로 인해 취약점이 발생했다.

1. **특정 요청 처리 시 검증 부족**
    - HFS는 HTTP 요청에서 전달되는 파라미터(예: 경로, 파일 이름 등)를 제대로 검증하지 않고 시스템 명령으로 전달했다.
    - 이를 통해 공격자가 악성 명령을 포함한 입력 값을 전달하면, 서버에서 이를 그대로 실행하게 되었다.
2. **`URL 인코딩` 해석**
    - 공격자는 URL 인코딩을 사용하여 특수 문자를 포함한 악성 명령을 숨기고, 이를 서버가 해석하도록 유도했다.
    - 예를 들어, URL 내에 `%00`(널 문자)나 `%26`(앰퍼샌드)을 삽입하여 명령 체인을 구성했다.
3. **명령 주입 공격**
    - 악성 HTTP 요청 예시:
        ...
        - 위 요청은 HFS 서버가 `calc`(계산기 실행) 명령을 실행하도록 유도한다.
        - `{.exec|COMMAND.}` 구문은 HFS의 특정 처리 메커니즘에서 악용될 수 있었다.

---

## 윈도우 애플리케이션 RCE 취약점 사례 - 침투 후에 악성 프로세스 확인

keyscan

### **취약점의 영향**

- **완전한 시스템 제어 가능**: 공격자는 임의의 명령을 실행할 수 있으므로, 파일 업로드, 데이터 탈취, 악성 코드 배포, 랜섬웨어 설치 등이 가능했다.
- **대규모 악용 사례**:
    - 이 취약점은 특정 해킹 도구 및 익스플로잇 키트에서 통합되어 사용되었다.
    - 취약한 HFS 서버를 대상으로 한 대규모 봇넷 공격의 일부로 활용되기도 했다.

아래와 같이 코드를 실행하면 공격 당하는 쪽에서 notepad가 실행이 된다.
![Untitled](https://github.com/user-attachments/assets/1acc0647-12b0-46e7-8d87-97889a09d4f3)

---

## crAPI 도커 웹 환경 해킹 (1) - crAPI 환경 구성

-crAPI는 가장 중요한 10가지 API 보안 위험을 이해하는 데 도움이 된다.
-crAPI는 애플리케이션은 설계상 모든 기능을 가장 안전한 방식으로 구현하지 않았다.
